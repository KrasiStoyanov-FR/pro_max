<template>
  <form class="flex flex-col gap-4 lg:flex-row lg:items-end" @submit.prevent>
    <div class="flex-1">
      <label class="block text-xs font-semibold uppercase tracking-wide text-neutral-400" for="sensors-search">
        Search
      </label>
      <div class="mt-1 input-field">
        <input
          ...
          type="search"
          name="search"
          placeholder="Search by name, ID, type, unit group, firmware..."
          :disabled="isLoading"
    ...
    <div class="flex flex-col">
      <label class="text-xs font-semibold uppercase tracking-wide text-neutral-400" for="group-by">
        Group by
      </label>
      <select
        id="group-by"
        ...
        :disabled="isLoading"
        :value="groupByValue"
        @change="emit('update:groupBy', ($event.target as HTMLSelectElement).value as SensorGroupByField)"
      >
        <option class="text-neutral-900" value="none">None</option>
        <option class="text-neutral-900" value="unitGroup">Unit Group</option>
        <option class="text-neutral-900" value="type">Type</option>
        <option class="text-neutral-900" value="status">Status</option>
      </select>
    </div>
  </form>
</template>
...
import type { SensorGroupByField } from '@/types/sensors'
...
const props = defineProps({
  ...
  groupBy: {
    type: String as () => SensorGroupByField,
    default: 'none'
  },
  isLoading: {
    type: Boolean,
...
const emit = defineEmits<{
  ...
  (e: 'update:groupBy', value: SensorGroupByField): void
}>()
...
const groupByValue = computed(() => (toValue(props.groupBy) ?? 'none') as SensorGroupByField)
</script>
