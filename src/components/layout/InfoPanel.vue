<template>
  <div
    class="flex flex-col bg-neutral-900/40 backdrop-blur-2xl transition-all duration-300 ease-out pointer-events-auto overflow-hidden"
    :class="[
      // Corner rounding based on cluster panel visibility
      props.hasClusterPanel ? '-ml-5 rounded-tr-2xl rounded-br-2xl' : 'rounded-2xl',
      // Size and visibility
      isOpen ? 'w-64 lg:w-80 opacity-100 translate-x-0 scale-100' : 'w-0 opacity-0 -translate-x-full scale-95',
      // Positioning - when no cluster panel, position at left edge
      !props.hasClusterPanel ? 'h-auto absolute top-4 bottom-4 lg:top-6 lg:bottom-6' : 'h-full'
    ]"
    :style="{ transitionDelay: isOpen ? '50ms' : '0ms' }">
    
    <!-- Content wrapper with proper visibility control -->
    <div class="flex flex-col h-full min-w-0 transition-opacity duration-200 ease-out"
         :class="[isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none']"
         :style="{ transitionDelay: isOpen ? '150ms' : '0ms' }">
      <!-- Panel header -->
    <div class="p-4 border-b border-neutral-400" :class="[props.hasClusterPanel ? 'pl-8' : '']">
    ...
    <!-- Panel content -->
    <div v-if="selectedPin" class="flex-1 overflow-y-auto p-4" :class="[props.hasClusterPanel ? 'pl-8' : '']">
      <!-- Target Type Badge -->
      <div class="mb-6">
    ...
    <!-- Empty state -->
    <div v-else class="flex-1 flex items-center justify-center p-4" :class="[props.hasClusterPanel ? 'pl-8' : '']">
