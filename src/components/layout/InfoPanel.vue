<template>
  <Transition
    ...
    enter-from-class="w-0 scale-95 opacity-0 translate-x-2"
    enter-to-class="w-64 lg:w-80 scale-100 opacity-100 translate-x-0"
    leave-active-class="transition-all duration-300 ease-in"
    leave-from-class="w-64 lg:w-80 scale-100 opacity-100 translate-x-0"
    leave-to-class="w-0 scale-95 opacity-0 translate-x-2"
  >
    <div
      data-map-overlay="left"
      class="h-full max-h-800 flex self-end p-4 z-10 rounded-2xl bg-neutral-900/40 backdrop-blur-3xl border border-white/10 transition-all duration-300 ease-out pointer-events-auto overflow-hidden"
      :class="[
        // Size and visibility
        isOpen ? 'w-64 md:min-h-60 lg:w-80 opacity-100 visible translate-x-0 scale-100 pointer-events-auto' : 'hidden w-0 h-0 opacity-0 -translate-x-full invisible scale-95 pointer-events-none',
        (!isOpen && hasClusterPanel) ? 'mr-0' : (isOpen && hasClusterPanel) ? 'mr-6' : '',
      ...
      <div class="h-full min-w-0 flex flex-col flex-1 transition-opacity duration-200 ease-out"
        ...
        <div class="flex items-start justify-between space-x-2">
          <div class="shape shape--outline shape--active" :class="[shape.shapeClass, `shape--${shape.value}`]"></div>

          <div class="flex-1 min-w-0">
            <template v-if="selectedPin">
              <h5 class="text-base font-semibold text-white">{{ selectedPin.type }}</h5>
              <h5 class="text-base font-semibold text-white">{{ selectedPin.title }}</h5>
            </template>
...
const shape: { value: MapPin['type']; shapeClass: string } = computed(() => {
  switch (props.selectedPin?.type) {
    case 'drone':
      return { value: 'drone', shapeClass: 'shape--circle' }
    case 'friendly':
      return { value: 'friendly', shapeClass: 'shape--square' }
    case 'sensor':
      return { value: 'sensor', shapeClass: 'shape--triangle' }
    default:
      return { value: 'unknown', shapeClass: 'shape--square' }
  }
})
